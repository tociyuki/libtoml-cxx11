...
struct toml_token_type : public layoutable, public check_builder_type {
    std::vector<int> base;
    std::vector<int> check;
    bool defined;

    toml_token_type () : check_builder_type (), base (), check (), defined (false) {}
    ...
    define_charset ()
    {
        ...
        charset ('\'', TOKEN_STRING);
        charset ('-', TOKEN_FIXNUM);
        charset ('+', TOLEN_FIXNUM);
        charset (',', TOKEN_COMMA);
        charset ('.', TOLEN_DOT);
        charset ('0','9', TOKEN_FIXNUM);
        charset ('=', TOKEN_EQUAL);
        charset ('[', TOKEN_LBRACKET);
        charset (']', TOKEN_RBRACKET);
        charset ('a','z', TOKEN_BOOLEAN);
