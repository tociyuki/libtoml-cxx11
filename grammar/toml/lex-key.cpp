...
struct toml_key_type : public layoutable, public check_builder_type {
    std::vector<int> base;
    std::vector<int> check;
    bool defined;

    toml_key_type () : check_builder_type (), base (), check (), defined (false) {}

    enum {WS=1,COMMENT, CR, LF, DQUOTE, COMMA, DOT, EQ,
          LBRACKET, RBRACKET, LBRACE, RBRACE, LETTER, MATCH};
    enum {PUTLIT = 1};
    ...
    define_state ()
    {
        ...
        to (5, MATCH, TOKEN_STRKEY);

        to (6, LETTER, 6, PUTLIT);
        to (6, MATCH, TOKEN_BAREKEY);
